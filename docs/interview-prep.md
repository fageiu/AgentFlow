...
> “评测体系是项目的一个特色。28 条 golden task 覆盖查询、政策知识检索、退款、审批、安全和幂等性 6 组能力。每条 case 明确声明必须和禁止调用的工具、审批行为、结构化 Outcome 以及最终沙箱状态。Judge 是纯规则化的：同一份 Trace 会得到相同评分，也不产生额外 Judge token。真实模型本身依然可能漂移，所以每次模型或 Prompt 变更都要重新跑回归，而不能把某次通过当成永久保证。并支持统计平均任务耗时、token消耗、平均工具调用次数，以及A/B 回归对比”
...
>
> 框架负责流程编排，但受控执行、工具校验、结构化outcome这些关键设计需要另外实现；代价是规划、重试、持久化和评测基础设施都要自己维护。"
