...
>
> 目标岗位：Agent 开发/AI 应用开发/AI 全栈，技术栈 TypeScript / Python / Node.js / Vue / AI Agent
...
### 30 秒版本（如果面试官说"简单介绍一下你的项目"）

> “企业工单 Agent 执行与评测平台，是一个 AI Agent 运行时和评测平台。核心架构是 Planner-Executor-Replanner：Planner 先生成计划，每步只授权一个工具；Executor 严格按计划执行，高风险操作走人工审批；工具失败时 Replanner 只重规划未完成步骤。服务端还会根据真实工具轨迹派生结构化 Outcome，不信任模型自报成功。28 条 golden task 用确定性规则检查工具轨迹、审批和业务副作用；真实模型仍可能漂移，所以模型或 Prompt 变更后必须重新回归。”
...
| 实验 | 结果 | 正确解读 |
| --- | --- | --- |
| [Mock 历史基线](evaluation-results/mock-full.md) | 18/18，约 6.2 秒 | 旧版 18 条用例的确定性基线，不代表当前 28 条真实模型能力 |
| [DeepSeek 28 条首轮全量](evaluation-results/deepseek-full-2026-07-20.md) | 26/28，92.9%，343,733 Token | 2 条知识检索用例因 Planner 输出不符合计划契约而异常 |
| [DeepSeek 28 条修复后全量](evaluation-results/deepseek-full-fixed-2026-07-20.md) | 28/28，100%，367,966 Token | 同一模型和 Prompt 版本下重新完成全量回归；这是一次实验结果，不代表未来运行不会漂移 |

推荐口径：先说首轮 28 条真实评测为 26/28，两个异常都来自 Planner 契约不合规；修复计划解析和约束后，第二轮完整 28 条回归达到 28/28。必须强调这是特定模型、Prompt 和代码版本下的一次完整实验，不把它包装成永久稳定性保证。
...
| **持久化** | 本地 JSON + PostgreSQL/pgvector（RAG） | 了解 — 区分 Demo 状态与知识索引边界 |
...
> 这八个点是你在面试中的“武器库”，每个都应该能展开讲解 5-10 分钟。
>
...
if (tool.riskLevel === 'high') {
  const rejectedToolMessage =
    yield * requestApprovalForTool(run, stepIndex++, toolCall, mode)
...
> “评测体系是项目的一个特色。28 条 golden task 覆盖查询、政策知识检索、退款、审批、安全和幂等性 6 组能力。每条 case 明确声明必须和禁止调用的工具、审批行为、结构化 Outcome 以及最终沙箱状态。Judge 是纯规则化的：同一份 Trace 会得到相同评分，也不产生额外 Judge token。真实模型本身依然可能漂移，所以每次模型或 Prompt 变更都要重新跑回归，而不能把某次通过当成永久保证。并支持统计平均任务耗时、token 消耗、平均工具调用次数，以及 A/B 回归对比”
...
```
政策文档 → 清洗/分块 → BGE-M3 Embedding → PostgreSQL/pgvector
                    └→ BM25 快照或 PostgreSQL 全文索引
                                      ↓
Agent Executor → searchPolicy → /v1/search → RRF 融合 → BGE 重排 → Top-K 证据
```
...
BM25 (关键词排序)        向量相似度 (语义排序)
   ...
   RRF 融合 (rrf_k=60)   ←  两个来源等权按排名累加
     ↓
   BGE Reranker（生产配置）或快速融合重排（演示配置）
     ↓
   分级阈值过滤 → Top-K 结果与 Citation
```

- **词法召回**：默认使用 BM25 内存快照，也可切换 PostgreSQL `tsvector`，保证政策编号和专业术语精确命中
- **向量召回**：使用 BGE-M3 Embedding 和 pgvector 覆盖同义表达
- **排序融合**：RRF 按 `1/(rrf_k + rank)` 累加两个来源的排名分，默认 `rrf_k=60`
- **重排与过滤**：生产配置使用 `bge-reranker-v2-m3`；普通结果、重排结果和未启用重排时的向量置信度分别使用不同阈值
- **证据输出**：只返回 Top-K 结果，并携带文件、版本、章节和页码 Citation，供 Trace 与 Outcome 引用

**评测保障：**

```python
# 代表性测试：覆盖精准匹配、语义泛化和边界拒绝
...
> "政策检索我们选择自建 RAG 混合检索，而不是直接把政策塞进 Prompt。原因有三：第一，企业政策有精确编号和条款，BM25 或 PostgreSQL 全文检索能保证专业术语不丢失。第二，用户表达和正式政策措辞可能不同，BGE-M3 向量召回可以覆盖同义表达。第三，我们先用 RRF 融合两个召回来源，再用 BGE Reranker 重排，最后按置信度过滤并输出带版本、章节和页码的 Citation。
>
> RAG 服务使用 Python FastAPI 实现，当前有 19 个 Python 源文件、约 2,450 行，并有 26 条 Python 测试。Agent 通过 `searchPolicy` 调用 `/v1/search`，服务模式失败时不会静默退回 Seed Policy；Fixture 只用于显式 Mock。TypeScript 与 Python 之间只通过 HTTP JSON 和 Citation 契约通信，因此 Executor 不需要感知底层检索实现。"
...
### Q1: 为什么不选择 LangGraph 等现有 Agent 框架？

**考察点**：技术选型能力、对现有生态的了解、独立判断力。

**60 秒回答：**

> “我们没有选择 LangGraph，并不是因为它做不到。LangGraph 本身支持状态图、持久化执行、流式输出和 Human-in-the-loop，能力比较完整。
>
> 选择自研主要和项目目标有关。这个项目不只是尽快搭建一个 Agent，还希望深入验证企业场景中的执行边界：每个计划步骤只能授权一个工具，工具风险等级必须由服务端决定，审批拒绝后不能继续写入，最终业务 Outcome 必须根据真实工具轨迹派生，不能相信模型自报成功。
>
> 如果使用 LangGraph，Planner、Executor、审批节点和 checkpoint 都可以建成图，但单步授权、Tool Registry、幂等语义、EvidencePacket、结构化 Outcome 和确定性评测仍然需要结合业务定制。对于当前规模，直接实现状态机能让关键执行协议更透明，也方便沿着 Trace 定位模型为什么越权、重试或产生错误副作用。
>
> 当然，自研也有明确代价：持久化恢复、分布式执行、可视化调试和生态集成都要自己建设。当前 Promise 审批不能跨进程恢复，就是这个取舍的体现。如果进入生产阶段，我会重新评估 LangGraph，让它承担 checkpoint、任务恢复和图编排，同时保留自己的 Tool Registry、业务安全约束、Outcome 和评测体系。”

**30 秒精简版：**

> “LangGraph 能做状态编排、持久化和人工审批，我们不选它不是因为能力不足，而是这个项目的目标之一就是深入实现和验证 Agent 执行协议。项目要求单步骤最小工具授权、服务端风险治理、可信 Outcome 和业务副作用评测，这些即使使用 LangGraph 也要定制。自研让核心状态流更透明，但代价是跨进程恢复和分布式调度要自己建设；生产化时可以让 LangGraph 承担通用编排，保留自己的业务安全层。”

**追问：LangGraph 不是也支持人工审批吗？**

> “是的，LangGraph 的 interrupt 和 checkpointer 原生支持暂停、保存状态和恢复执行，不能把‘不支持审批’作为选型理由。区别在于我们的审批还绑定了 Tool Registry 风险等级、拒绝后的写入阻断、退款补偿、幂等语义和可信 Outcome。这些属于业务安全规则，无论使用什么编排框架都要自己实现。”

**回答时要避免：**

- 不说“LangGraph 做不到审批、流式执行或状态恢复”。
- 不笼统地说“自研更灵活”，要落到单步授权、可信 Outcome 和业务副作用评测。
- 不把自研描述成没有成本；要主动说明当前跨进程恢复和分布式调度的不足。
- 不把框架和业务安全层对立起来；生产方案可以采用“LangGraph 通用编排 + 自有业务约束层”。

> **一句话收束：**“框架解决的是通用编排问题，但不能替代业务安全边界；我的选型重点是明确哪些能力交给框架，哪些事实必须由确定性代码负责。”
...
> "RAG 检索服务用 Python FastAPI 实现，提供 `/v1/search` 端点。Agent 端通过 `searchPolicy` 工具调用它；该工具和其他业务工具一样注册在 Tool Registry 中，统一经过参数校验、风险识别和 Trace 记录。
>
> **为什么用 Python 而非 TypeScript？** Python 的 NLP/ML 生态更成熟，项目可以直接复用 LlamaIndex、BGE Embedding、CrossEncoder Reranker 和 BM25 组件；TypeScript 侧继续负责 Agent 编排和业务安全边界。
>
> **集成方式**：TypeScript 客户端通过 HTTP JSON 请求 RAG 服务，把响应转换为统一的 Policy Citation 契约，再作为 tool result 进入 Agent 上下文。客户端复用超时、有限重试、取消信号和错误映射；真实 Service 模式失败时禁止静默回退 Fixture。
>
> **混合检索策略**：词法侧可以使用 BM25 或 PostgreSQL 全文召回，向量侧使用 BGE-M3 与 pgvector；两路结果先用 RRF 融合，再经过 BGE Reranker 或演示模式快速重排。当前有 26 条 Python 测试覆盖检索、索引、文档生命周期和边界错误。"
...
> 3. **重排与过滤**：生产配置使用 BGE CrossEncoder 重排；最低融合分、重排分默认是 0.35，关闭重排时还要求最高向量分达到 0.55，避免低置信度结果进入 Agent 上下文
>
...
| **Result** | 系统向模型暴露 8 种业务工具，并保留 2 个演示控制工具；越权调用会被 Executor 拒绝并记录失败 Trace。核心执行器约 1,600 行 TypeScript。这里不声称模型越权概率为零，只说明服务端不会放行已识别的越权调用。 |
...
| **Result** | 旧版 Mock 基线为 18/18；扩展到 28 条后，DeepSeek 首轮全量评测为 26/28，两个异常都来自 Planner 输出不符合计划契约。修复后重新执行完整 28 条用例取得 28/28。这个结果证明 Trace 和规则 Judge 能定位并验证修复，但不代表真实模型从此不会发生行为漂移。 |
...
| **Action** | 我选择自建 RAG 服务而不是接入第三方知识库。用 Python FastAPI 搭建检索服务（19 个源文件，约 2,450 行）。采用混合检索策略：BM25 或 PostgreSQL 全文检索保证政策编号和术语精确命中，BGE-M3 与 pgvector 覆盖语义召回，RRF 融合后通过 BGE Reranker 重排。索引管线支持版本、校验和与 current 文档切换。Agent 端通过 `searchPolicy` 和 `/v1/search` 集成，评测包含 26 条 Python 测试和 15 条 Agent 端 knowledge case。 |
...
>
> 8. **企业政策 RAG 混合检索**：自建 Python FastAPI 检索服务（约 2,450 行），BM25/PostgreSQL 全文召回与 BGE-M3 向量召回经 RRF 融合，再由 BGE Reranker 重排。通过 `searchPolicy` 工具与 Agent 集成，26 条 Python 测试和 15 条 Agent 端 knowledge case 保障检索质量。
...
### 第一阶段：熟悉代码和架构（第 1-7 天）

| 天数 | 任务 | 具体内容 |
| --- | --- | --- |
| 第 1 天 | 整体概览 | 通读 README.md、docs/architecture.md，理解项目全貌和执行链路 |
| 第 2 天 | 核心执行器 | 精读 executor.ts（约 1,600 行），理解 Planner-Executor-Replanner 循环 |
| 第 3 天 | Tool Registry + 业务决策 | 精读 toolRegistry.ts 和 businessDecision.ts，理解双重校验和证据提取 |
| 第 4 天 | LLM Provider | 精读 provider.ts（约 900 行），理解模型封装和 Mock 机制 |
| 第 5 天 | 评测体系 | 精读 eval/\*.ts 中的 evaluationRunner、evaluationScorer、evaluationCases（28 条） |
| 第 6 天 | RAG 检索服务 | 快速浏览 apps/rag/src/，精读 retrieval.py（混合检索实现） |
| 第 7 天 | 整体复盘 | 画出完整的执行链路图（含 RAG 集成），确保能脱稿讲清每个环节 |

### 第二阶段：准备面试话术（第 8-14 天）

| 天数 | 任务 | 具体内容 |
| --- | --- | --- |
| 第 8 天 | 亮点一 | 准备 Planner-Executor-Replanner 的 5 分钟讲解 |
| 第 9 天 | 亮点二 | 准备 Tool Registry + 双重校验的讲解 |
| 第 10 天 | 亮点三 | 准备 Human-in-the-loop 审批流的讲解 |
| 第 11 天 | 亮点四 | 准备工具失败重试 + 恢复校验的讲解 |
| 第 12 天 | 亮点五 | 准备确定性评测体系（28 条用例，6 组能力）的讲解 |
| 第 13 天 | 亮点六 + 七 | 准备结构化业务决策 + Provider / Mock 讲解 |
| 第 14 天 | 亮点八 + 整合 | 准备 RAG 混合检索讲解，并把八个亮点串成 10 分钟项目总览 |

### 第三阶段：模拟面试（第 15-21 天）

| 天数 | 任务 | 具体内容 |
| --- | --- | --- |
| 第 15-17 天 | 高频问题 | 练习 Q1-Q9 的答案，每个控制在 3-5 分钟 |
| 第 18-19 天 | STAR 故事 | 练习四个 STAR 故事，每个 3-5 分钟 |
| 第 20 天 | 追问准备 | 熟悉 12 个追问方向的应对思路（含 RAG、业务证据、跨语言架构） |
| 第 21 天 | 全真模拟 | 找朋友或 AI 做一次完整模拟面试 |
