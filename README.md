...
在线 Fast 模式以 BGE-M3 向量分与 RRF 保留 Top-2，Top 3–5 按融合排名保护多政策覆盖；
完整模式再使用 BGE CrossEncoder 选择 Top-1，并采用相同的融合覆盖策略。
普通融合候选使用 `RAG_MINIMUM_SCORE`，完整重排主证据使用独立的
`RAG_MINIMUM_RERANK_SCORE`。完整模式由 Reranker 选择 Top-1，Top 2–5 按高置信
融合排名补齐并按文档去重，避免多政策问题因重排而丢失必要依据。
...
评测 JSON 除顶层 Recall、MRR、拒答率和延迟汇总外，还保存每条 Query 的预期/实际
Policy ID、融合原始排名、重排后排名以及各阶段分数，用于准确判断重排增益和召回损失。
评测客户端通过 `include_diagnostics` 显式请求这些阶段明细，普通 Agent 检索不返回候选账本，
避免扩大在线响应与 Trace 体积。
单条 HTTP 超时或服务错误会写入对应 Case 的 `error` 并累计 `error_count`，继续执行剩余
Query；只要存在错误，Fast/Full 门禁都不会通过。
未通过的目标政策会自动归类为 `not_recalled`、`ranking_dropped`、`top1_incorrect`
或 `top1_regression`，顶层 `diagnosis_counts` 汇总各类数量，便于区分语料召回与排序问题。
