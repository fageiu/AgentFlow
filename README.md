...
- Evaluation Workbench：内置 12 条评测用例，支持按能力分组运行、断言诊断、工具轨迹、token/工具调用指标和模型/Prompt A/B 对比。
...
- Week 11：评测系统 MVP，支持内置用例、批量执行、规则评分、评测历史和前端结果面板。
- Week 12：评测结果详情与回归对比，支持能力分组、局部评测、断言诊断、工具轨迹和最近两轮趋势判断。
- Week 13：对齐原规划补齐评测指标，内置 12 条 golden task，展示平均耗时、平均工具调用、token 消耗、失败原因和模型/Prompt A/B 对比。
